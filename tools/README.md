# dEQP Auto Test Tools

自动化 dEQP (drawElements Quality Program) 测试工具，用于通过串口控制嵌入式设备执行 OpenGL ES 一致性测试。

## 📁 目录结构

```
tools/
├── run_auto_test.py      # 主入口脚本
└── lib/
    ├── __init__.py       # 模块导出
    ├── test_parser.py    # XML 测试用例解析器
    ├── serial_utils.py   # 串口通信工具
    ├── device_control.py # 设备控制（重置、存活检测）
    ├── test_runner.py    # 测试执行逻辑
    └── utils.py          # 通用工具函数
```

## 🚀 功能特性

- **XML 解析**: 解析 dEQP 测试用例 XML 文件，提取测试组和测试用例
- **串口通信**: 通过串口向嵌入式设备发送测试命令
- **自动重置**: 每个测试组执行后自动重置设备
- **状态检测**: 检测测试完成、系统崩溃（PANIC）、系统挂起等状态
- **断点续测**: 支持从指定测试组开始，跳过已完成的测试
- **日志记录**: 为每个测试组生成独立的日志文件
- **CSV 报告**: 生成包含所有测试结果的 CSV 报告
- **进度显示**: 实时显示测试进度、统计信息和时间

## 📋 依赖要求

```bash
pip install pyserial
```

## 📖 使用方法

### 基本命令

```bash
cd tools

# 查看帮助
python3 run_auto_test.py -h

# 显示测试用例结构（默认深度）
python3 run_auto_test.py -f ../dEQP-GLES2-cases.xml

# 显示摘要信息
python3 run_auto_test.py -f ../dEQP-GLES2-cases.xml --summary

# 列出所有测试用例
python3 run_auto_test.py -f ../dEQP-GLES2-cases.xml --list-tests

# 列出所有测试组
python3 run_auto_test.py -f ../dEQP-GLES2-cases.xml --list-groups
```

### 运行测试

```bash
# 基本测试运行
python3 run_auto_test.py -f ../dEQP-GLES2-cases.xml \
    --run-tests \
    --test-port /dev/ttyUSB1

# 完整测试运行（带设备重置）
python3 run_auto_test.py -f ../dEQP-GLES2-cases.xml \
    --run-tests \
    --test-port /dev/ttyUSB1 \
    --reset-port /dev/ttyCH341USB1

# 从指定组开始测试（断点续测）
python3 run_auto_test.py -f ../dEQP-GLES2-cases.xml \
    --run-tests \
    --test-port /dev/ttyUSB1 \
    --reset-port /dev/ttyCH341USB1 \
    --start-group "dEQP-GLES2.functional.shaders"

# 打印设备输出日志
python3 run_auto_test.py -f ../dEQP-GLES2-cases.xml \
    --run-tests \
    --test-port /dev/ttyUSB1 \
    --print-output
```

## ⚙️ 命令行参数

### 输入文件

| 参数 | 说明 |
|------|------|
| `-f, --file` | XML 测试用例文件路径（必需） |

### 显示选项

| 参数 | 说明 |
|------|------|
| `-d, --depth` | 显示的最大深度（默认：全部） |
| `--list-tests` | 列出所有可执行测试用例路径 |
| `--list-groups` | 列出所有测试组路径 |
| `--summary` | 仅显示摘要信息 |

### 测试执行

| 参数 | 说明 |
|------|------|
| `--run-tests` | 按组运行测试 |
| `--test-port` | 测试命令串口（如 /dev/ttyUSB0） |
| `--test-baudrate` | 测试串口波特率（默认：921600） |
| `--test-timeout` | 测试响应超时秒数（默认：10） |

### 设备重置

| 参数 | 说明 |
|------|------|
| `--reset-port` | 重置控制串口（如 /dev/ttyUSB1） |
| `--reset-baudrate` | 重置串口波特率（默认：9600） |
| `--reset-wait` | 重置后等待时间秒数（默认：5） |

### 测试控制

| 参数 | 说明 |
|------|------|
| `--max-wait-count` | 最大等待尝试次数（默认：10） |
| `--log-dir` | 日志目录（默认：自动创建带时间戳的目录） |
| `--start-group` | 从指定组开始测试（跳过之前的组） |
| `--print-output` | 打印设备串口输出（默认：禁用） |

## 📊 测试结果

### 结果状态

| 状态 | 说明 |
|------|------|
| `PASSED` | 测试成功完成（检测到 "DONE!"） |
| `CRASH` | 系统崩溃（检测到 "PANIC"） |
| `HANG` | 系统无响应（free 命令无响应） |
| `TIMEOUT` | 超时（超过最大等待次数） |

### 输出文件

测试完成后，在日志目录中生成：

```
logs_YYYYMMDD_HHMMSS/
├── test_report.csv                              # CSV 测试报告
├── dEQP-GLES2_functional_shaders_xxx.log       # 各测试组日志
└── ...
```

### CSV 报告格式

```csv
Index,Group Path,Result,Duration (s),Start Time,End Time
1,dEQP-GLES2.functional.shaders.xxx,PASSED,15.32,2025-12-12 10:00:00,2025-12-12 10:00:15
...

# Summary
Total Groups,941
Skipped,0
To Test,941
Completed,100
Passed,95
Failed,0
Timeout,3
Hang,1
Crash,1
Pass Rate (%),95.0
Total Time (s),3600.00
Avg Time per Group (s),36.00
```

## 🔧 设备重置协议

默认使用以下 HEX 命令进行设备重置：

- **上电命令**: `A0 01 01 A2`
- **断电命令**: `A0 01 00 A1`

如需修改，请编辑 `lib/device_control.py` 中的 `reset_device()` 函数。

## 📝 示例输出

```
2025-12-12 10:00:00 - INFO - ============================================
2025-12-12 10:00:00 - INFO - Total leaf groups to test: 941 (skipped: 0)
2025-12-12 10:00:00 - INFO - ============================================
2025-12-12 10:00:00 - INFO - Log directory: logs_20251212_100000
2025-12-12 10:00:00 - INFO - Writing test report to: logs_20251212_100000/test_report.csv

2025-12-12 10:00:05 - INFO - ================================================
2025-12-12 10:00:05 - INFO - [1/941] (#1/941) Testing group: dEQP-GLES2.xxx
2025-12-12 10:00:05 - INFO - ================================================
2025-12-12 10:00:05 - INFO - Writing log to: logs_20251212_100000/dEQP-GLES2_xxx.log
2025-12-12 10:00:20 - INFO - Group dEQP-GLES2.xxx completed successfully.
2025-12-12 10:00:20 - INFO - Case duration: 15.3s

2025-12-12 10:00:20 - INFO - Progress: [█░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░] 1/941 (0.1%)
2025-12-12 10:00:20 - INFO - Results:  ✅ Passed: 1  ❌ Failed: 0  ⏱ Timeout: 0  💀 Hang: 0  💥 Crash: 0
2025-12-12 10:00:20 - INFO - Time:     ⏱ Case: 15.3s  📊 Total: 15.3s
```

## 🏗️ 模块说明

### test_parser.py

- `TestCase`: 测试用例数据类
- `TestCaseParser`: XML 解析器，提供测试路径收集方法

### serial_utils.py

- `serial_open()`: 打开串口
- `serial_write()`: 发送字符串命令
- `serial_write_hex()`: 发送 HEX 数据
- `serial_wait_for_response()`: 等待关键字响应

### device_control.py

- `check_system_alive()`: 发送 `free` 命令检测系统存活
- `reset_device()`: 发送重置命令并等待设备启动

### test_runner.py

- `build_test_command()`: 构建 openglcts 测试命令
- `run_group_tests()`: 主测试执行循环

### utils.py

- `format_duration()`: 格式化时间显示
- `print_title_info()`: 打印带边框的标题
- `print_progress()`: 打印进度条和统计信息
